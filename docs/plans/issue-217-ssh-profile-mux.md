...
| --- | --- | --- | --- |
| Preserve explicit-host `remote_bash`, safe mux execution, and local built-in `bash` | characterization | todo | Existing `remote-bash.test.ts` and extension tool-name contract |
| `ssh_profile list/use/status/clear` over parsed `~/.ssh/config` aliases; persist active defaults in the Pi session branch | profile | todo | Pending profile/tool integration tests |
