...
| Keyed best-effort footer shows active target/workdir and mux up/down; clears on profile clear/shutdown | footer lifecycle | done | `packages/pi-better-ssh/src/e2e.test.ts:66`; focused extension suite passed 5/5 and asserts no `bash` override |
| Package, core consumer, and repository verification are honestly green | closeout | in-progress | Pending full validation and proof artifacts |
