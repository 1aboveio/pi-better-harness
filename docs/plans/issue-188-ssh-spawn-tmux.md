...
| Round 2 `ssh-tmux-stop-race`: stop during an in-flight detached session start waits for the start attempt, kills the named remote session, cancels, and remains callback-silent | tmux start/stop concurrency | done | RED: 2 remote calls, no `kill-session`; GREEN: regression test plus full runtime 20/20 and preset 19/19 |
| Coverage and close-out gates | generated evidence and PR contract | in-progress | runtime smoke and checklist validation pass; exact-HEAD gates and PR contract pending |
