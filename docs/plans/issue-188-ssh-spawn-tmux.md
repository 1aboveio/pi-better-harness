...
| --- | --- | --- | --- |
| AC1, AC2, AC5: default tmux mode bootstraps before a stable detached session start and fails closed | tmux launch lifecycle | in-progress | pending TDD integration test |
| AC3, AC8: remote output reaches the durable local log and normal success/failure callbacks remain active | tmux supervision | todo | pending TDD integration tests |
