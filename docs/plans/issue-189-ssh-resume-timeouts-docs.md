...
| Operator-facing contract | Compact launch/status identify host and mode; verbose status retains non-secret support metadata; docs/tool descriptions cover structured SSH, tmux bootstrap/install/needs-user, direct watch/escape hatch, fail-closed behavior, and no ControlMaster | in-progress | tool contract GREEN; package docs pending |
