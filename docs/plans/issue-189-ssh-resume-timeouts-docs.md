...
| Close-out evidence | Runtime smoke, scope classification, ledger/checklist, lint sweeps, scan-diff, exact-head evidence, CI, and PR Review Contract | done | PR #194 opened with full contract; local/root suites and smoke passed; scope is surface-bearing/highRisk false; lint and scan findings are clean; checklist validates; first pushed head CI passed and final-head evidence is recorded in the PR |
| Round 2 `ssh-timeout-status` | A deadline-bounded tmux supervision timeout, including an empty timed-out poll response, must finalize `timed_out` and attempt `kill-session`; malformed protocol before the deadline must remain `failed` | done | RED reproduced `failed` without `kill-session`; GREEN: focused 2/2, runtime 37/37, preset 20/20, package 95/95, and package typecheck passed; #188 in-flight start/stop regression remains green |
