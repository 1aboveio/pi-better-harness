...
| No un-specced capability missed — the surfaces no AC mentions | judgment | PASS | Only `git-remotes.ts` library surfaces change (`readGitRemotes`, `syncGitRemotes`). No CLI/tool/route/sandbox/CI-delivery surface is added. Issue #109 ACs name exactly push-only remote topology + source-read-failure non-mutation on those two operations. #103 normal topologies stay covered by the existing suite in `tests/git_remotes.test.mjs`. |
