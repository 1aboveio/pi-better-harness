...
`pi-better-harness` is a Pi meta package that installs the core Pi Better Harness extensions: an opt-in foreground write sandbox, delegated subagents, durable background tasks, goal tracking, and structured execution plans.
...
- `pi-better-plan` for persistent plan progress and `→` keyboard navigation.
...
Use the installer when you want every core extension with standalone package identities. Install an individual package instead when you only need the sandbox, subagents, shell task supervision, goal tracking, or plan progress.
